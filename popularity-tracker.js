...
exports.checkAndStore = async () => {
  try {
    ...
    const tweet = await politicianTwitter.get("users/lookup", {
      user_id: idLookupString
    });
    for (let i = 0; i < tweet.length; i++) {
      const candidate = await db.collection("candidates").findOneAndUpdate(
        ...
        {
          $push: {
            data: { followerCount: tweet[i].followers_count, date: new Date() }
          },
          $set: {
      ...
      if (!candidate) {
        ...
        console.log(
          `Found and updated info for: ${JSON.stringify(
            candidate.value.handle
          )}. Now has ${JSON.stringify(
            candidate.value.data[0].followerCount
          )} followers`
        );
      }
    }
    ...
    for (let i = 0; i < tweet.length; i++) {
      const atlasCandidate = await atlasDb
        .collection("handles")
        .findOneAndUpdate(
          { id_str: tweet[i].id_str },
          {
            $push: {
              data: {
                followerCount: tweet[i].followers_count,
                date: new Date()
              }
            },
            $set: {
      ...
      if (!atlasCandidate) {
        ...
        console.log(
          `Found and updated Atlas info for: ${JSON.stringify(
            atlasCandidate.value.handle
          )}. Now has ${JSON.stringify(
            atlasCandidate.value.data[0].followerCount
          )} followers`
        );
      }
    }
    ...
    console.log(
      "Error with checkAndStoreAll for Atlas: " + JSON.stringify(err)
    );
  }
};
