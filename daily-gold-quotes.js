  ...
  try {
    ...
    request.get(goldChartSrc, (err, res, body) => {
      fs.writeFile(`${todaysDate}goldchart.png`, body, err => {
        ...
        dailygoldquotes.post(
          "media/upload",
          { media: goldChartBuffer },
          (err, media, res) => {
            if (!err) {
              const status = {
      ...
      request.get(silverChartSrc, (err, res, body) => {
        fs.writeFile(`${todaysDate}silverchart.png`, body, err => {
          if (err) throw err;
          console.log("The file was saved");
          const goldChartBuffer = fs.readFileSync(
            `./${todaysDate}silverchart.png`
          );
          //Upload chart png file
          dailygoldquotes.post(
            "media/upload",
...
exports.followFollowers = async () => {
  try {
    ...
    console.log(
      "Error following followers of @dailygoldquotes: " + JSON.stringify(err)
    );
  }
};
